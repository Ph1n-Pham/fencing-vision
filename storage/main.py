    ...
    if is_valid:
        ...
        if st.button('Detect'):
            with rd.stderr(format='markdown', to=st.sidebar), st.spinner('Wait for it...'):
                print(subprocess.run(['yolo', 'task=segment', 'mode=predict', 'model=trained_models/best.pt', 'conf=0.25', 'source={}'.format(source)],capture_output=True, universal_newlines=True).stderr)
